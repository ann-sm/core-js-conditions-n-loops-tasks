...
function canQueenCaptureKing(queen, king) {
  const xDif = Math.abs(queen.x - king.x);
  const yDif = Math.abs(queen.y - king.y);
  return xDif === 0 || yDif === 0 || xDif === yDif;
}
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    switch (numberStr[i]) {
      ...
      case '9':
        ...
        break;
      case '0':
        result = `${result}${space}zero`;
        break;
      case '-':
        result = `${result}${space}minus`;
        break;
      case '.':
      case ',':
        result = `${result}${space}point`;
        break;
      default:
        break;
    }
...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < newArr.length; i += 1) {
    ...
    for (j = i - 1; j >= 0 && newArr[j] > current; j -= 1) {
      newArr[j + 1] = newArr[j];
    }
    newArr[j + 1] = current;
  }
